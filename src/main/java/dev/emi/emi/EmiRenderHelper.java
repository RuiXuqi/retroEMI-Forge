...
public class EmiRenderHelper {
    ...
    public static void drawTintedSprite(MatrixStack matrices, String sheet, int icon, int color, int x, int y, int xOff, int yOff, int width, int height) {
        ...
        buffer.pos(xMin, yMax, 0).tex(uMin, vMax).endVertex();
        buffer.pos(xMax, yMax, 0).tex(uMax, vMax).endVertex();
        buffer.pos(xMax, yMin, 0).tex(uMax, vMin).endVertex();
        buffer.pos(xMin, yMin, 0).tex(uMin, vMin).endVertex();
        tess.draw();
        RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, 1.0f);
    }
