    ...
    private static void invitePlayer() {
        ...
        for (EntityPlayer player : client.world.playerEntities) {
            if (player.getDisplayNameString().equals(name)) {
                get().opponent = player.getDisplayNameString();
                sendNetwork(player.getDisplayNameString(), -1, 0, 0);
            }
        }
    }
    ...
    public static void receiveNetwork(String uuid, int type, int start, int end) {
        ...
        if (type == -1) {
            if (EmiScreenManager.hasSidebarAvailable(SidebarType.CHESS)) {
                chess.pending = uuid;
                client.player.sendMessage(new TextComponentString(EmiPort.translatable("emi.chess.multiplayer.invited", player.getDisplayNameString()).asString()));
            } else {
            ...
            if (chess.started) {
                ...
                if (uuid.equals(chess.opponent)) {
                    client.player.sendMessage(new TextComponentString(EmiPort.translatable("emi.chess.multiplayer.accepted", player.getDisplayNameString()).asString()));
                    chess.generator = new NetworkedMoveGenerator(PieceColor.BLACK);
                }
            }
        } else if (type == -3) {
            if (uuid.equals(chess.opponent)) {
                client.player.sendMessage(new TextComponentString(EmiPort.translatable("emi.chess.multiplayer.cancelled", player.getDisplayNameString()).asString()));
                restart();
            }
        } else if (type == -4) {
            if (uuid.equals(chess.opponent)) {
                client.player.sendMessage(new TextComponentString(EmiPort.translatable("emi.chess.multiplayer.unavailable", player.getDisplayNameString()).asString()));
            }
